...
function blur(src, srcW, srcH, method/*, radius*/) {
  var x, y,
      rowPtr,
  ...
  if (method === 'box-blur') {
    ...
    for (y = 0; y < srcH; y++) {
      rowPtr = y * srcW;
      output[rowPtr] = (src[rowPtr] + src[rowPtr + 1]) >> 1;
      for (x = 1; x < srcW-1; x++) {
        output[rowPtr + x] = (src[rowPtr + x - 1] + src[rowPtr + x] + src[rowPtr + x + 1]) / 3;
      }
      output[rowPtr + x] = (src[rowPtr + x] + src[rowPtr + x - 1]) >> 1;
    }
    // similar vertical motion blur
    for (x = 0; x < srcW; x++) {
      output[x] = (src[x] + src[x + srcW]) >> 1;
      for (y = 1; y < srcH-1; y++) {
        output[y * srcW + x] = (src[(y-1) * srcW + x] + src[y * srcW + x] + src[(y+1) * srcW + x]) / 3;
      }
      output[y * srcW + x] = (src[y * srcW + x] + src[(y - 1) * srcW + x]) >> 1;
    }
  } else {
    // default blur method - gaussian
    for (x = 0; x < srcW; x++) {
      for (y = 0; y < srcH; y++) {
